#!/bin/bash
# ==============================================================================
# Dr. GRPO training — Llama-3.2-3B-Instruct  (full RL + layer-wise RL sweep)
# ==============================================================================
# Adapted from run_drgrpo_qwen25math_1.5b_tmux.sh.  Key differences vs Qwen 1.5B:
#   - Model:    meta-llama/Llama-3.2-3B-Instruct  (28 transformer layers, idx 0..27)
#   - Batches:  micro 8 → 4 (3B is ~2× larger than 1.5B; micro=4 fits H100 80GB
#               comfortably with FSDP across 8 GPUs)
#   - Rollout:  TP=1 (3B fits single-GPU vllm easily; no need for TP)
#   - Data:     same math_drgrpo dataset; verl auto-applies Llama-3.2 chat template
#               to the [{role, content}, ...] prompt column at runtime.
#
...
# ------------------------------------------------------------------------------
# SINGLE experiment:
#   bash run_drgrpo_llama32_3b_instruct_tmux.sh                 # full RL (all params)
#   bash run_drgrpo_llama32_3b_instruct_tmux.sh --layer 14      # train only layer 14
#   bash run_drgrpo_llama32_3b_instruct_tmux.sh --layer 0,14,27
#
# SWEEP across settings (sequential on one node):
#   bash run_drgrpo_llama32_3b_instruct_tmux.sh --layers "full 0 7 14 21 27"
#
...
#   EPOCHS=5 bash run_drgrpo_llama32_3b_instruct_tmux.sh --layers "0..27" --part 1/4 --resume
...
# ===== defaults =====
MODEL_PATH="${MODEL_PATH:-meta-llama/Llama-3.2-3B-Instruct}"   # HF repo name → auto-download (or pass --model /local/path)
MODEL_TAG="${MODEL_TAG:-Llama-3.2-3B-Instruct}"
...
WANDB_MODE="${WANDB_MODE:-online}"   # "offline" for no-network, "disabled" to skip wandb entirely
NO_TMUX=false; EXTRA_ARGS=()
LAYER="${LAYER:-}"      # single-experiment layer id(s); "" = full RL
LAYERS="${LAYERS:-}"    # sweep list, e.g. "full 0 7 14 21 27" or "full 0..27"
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    ...
    ENV_INJECT="LR=${LR:-} EPOCHS=${EPOCHS:-} BATCH_SIZE=${BATCH_SIZE:-} MINI_BATCH=${MINI_BATCH:-} MICRO_BATCH=${MICRO_BATCH:-} ROLLOUT_N=${ROLLOUT_N:-} ROLLOUT_TP=${ROLLOUT_TP:-}"
...
# ===== Dr. GRPO hyperparams (paper Table 6) — tuned for 3B =====
LR="${LR:-1e-6}"         # paper default; Instruct model is more sensitive than base, keep paper lr
...
MICRO_BATCH="${MICRO_BATCH:-4}"           # 1.5B used 8; 3B is ~2× bigger → micro=4 keeps activation budget similar
ROLLOUT_TP="${ROLLOUT_TP:-1}"             # 3B fits single-GPU vllm easily, TP=1 is optimal
EPOCHS="${EPOCHS:-5}"
...
============================================================
  ...
  batch/mini/μ  : $BATCH_SIZE / $MINI_BATCH / $MICRO_BATCH    rollout n: $ROLLOUT_N   rollout TP: $ROLLOUT_TP
